...
if ($db->connect_error) {
  die("Error Occurred " . $db->connect_error);
} 
?>
            ...
            */ 
			 ...
			 $image=$db->query("SELECT * FROM imagedetails");
              while($row = $image->fetch_assoc()) {
                if ( !array_key_exists("continent", $_GET) or (($row['ContinentCode']== $_GET['continent'] or $_GET['continent']==="0") 
					and ($row['CountryCodeISO']==$_GET['country'] or $_GET['country']==="0") and 
				($_GET['title']=="" or $_GET['title']==$row['Title']))){
                  echo "<li>
